use super::init_data::init_entity_pool_res;
use crate::{
    components::{GameStateComponent, LoadingStateComponent},
    systems::{
        create::{
            create_3d_camera_event, create_map_system, create_player_entity_system,
            create_player_system,
        },
        entities::{
            find_world_entities_in_range_system, move_entities_event, render_entities_system,
        },
        state_management::{state_build, state_cleanup},
    },
    AppState,
};
use bevy::{
    ...
    prelude::{
        in_state, Bundle, Camera3dBundle, Component, EventReader, IntoSystemConfigs, KeyCode,
        OnEnter, Query, Res, With,
    },
...
pub struct GamePlayPlugin; // TODO
impl Plugin for GamePlayPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(
            OnEnter(AppState::Game),
            (
                state_build::<GameStateComponent>,
                // Build world plugin
                create_3d_camera_event::<GameStateComponent>,
                create_player_entity_system,
                init_entity_pool_res,
                (
                    create_map_system,
                    find_world_entities_in_range_system,
                    render_entities_system,
                    create_player_system,
                ),
                state_cleanup::<LoadingStateComponent>,
            )
                .chain(),
        )
        .add_systems(
...
fn mouse_capture(
    ...
    let mut total_scroll_lines: Vec2 = Vec2::ZERO;
    let mut total_scroll_pixels: Vec2 = Vec2::ZERO;
...
pub fn key_capture(
    ...
    mut camera_query: Query<(&PanOrbitSettings, &mut PanOrbitState, &mut Transform)>,
    mut q_windows: Query<&mut Window, With<PrimaryWindow>>,
) {
    let mut window = q_windows.get_single_mut().unwrap();
    // window.cursor.grab_mode = CursorGrabMode::Locked;
    window.cursor.visible = false;
    let camera_transform = &mut camera_query.get_single_mut().unwrap().2;

    let mut move_increment = Vec3::ZERO;

    if keyboard_input.pressed(KeyCode::KeyW) {
        move_increment += Vec3::new(0.0, 0.0, -0.5);
    }
    if keyboard_input.pressed(KeyCode::KeyA) {
        move_increment += Vec3::new(-0.5, 0.0, 0.0);
    }
    ...
    if keyboard_input.pressed(KeyCode::KeyQ) {
        move_increment += Vec3::new(0., 0.5, 0.0);
    }
    if keyboard_input.pressed(KeyCode::KeyE) {
        move_increment -= Vec3::new(0., 0.5, 0.0);
    }

    let rotated_move = camera_transform.rotation * move_increment;
    camera_transform.translation += rotated_move;
}
